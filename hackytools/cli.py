...
def hackystats(args):
    ...
    cpu_path = lambda i: f"/sys/devices/system/cpu/cpu{i}/cpufreq/scaling_cur_freq"
    temps = "/sys/class/thermal/thermal_zone0/temp"

    x = os.cpu_count()

    print(f"\x1b[4mCPU Freqs ({x})\x1b[0m")
    for i in range(x):
        with open(cpu_path(i)) as f:
            freq = int(f.read().strip())
        print(f"  CPU{i}: {freq/1000:,.0f} MHz")
